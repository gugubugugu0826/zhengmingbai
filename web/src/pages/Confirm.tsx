    ...
    <div className="px-5 pb-3 pt-3">
      ...
      <div className="flex gap-1.5">
        {[0, 1, 2].map((i) => (
          <div
            ...
            className={`h-2 flex-1 rounded-full transition-all duration-700 ${
              i < step
                ? 'bg-primary'
                : i === step && animating
                  ? 'relative overflow-hidden bg-primary/30'
                  : i === step
                    ? 'bg-primary'
                    : 'bg-soft'
            }`}
          >
            {i === step && animating && (
              <div className="absolute inset-0 -translate-x-full animate-[shimmer_1.5s_ease-in-out_infinite] bg-gradient-to-r from-transparent via-primary/40 to-transparent" />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="mt-1.5 flex justify-between px-0.5">
        {['AI 分组', 'AI 识别', 'AI 方案'].map((label, i) => (
          <span
            key={label}
            className={`text-[10px] transition-colors ${
              i <= step ? 'font-medium text-primary' : 'text-warm-light'
            }`}
          >
            {label}
          </span>
