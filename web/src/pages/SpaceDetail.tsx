...
export default function SpaceDetailPage(): JSX.Element {
  ...
  const [confirmDelete, setConfirmDelete] = useState<SpaceHistoryItem | null>(null);
  ...
  useEffect(() => { loadData(); }, [spaceId, navigate]);
  ...
  const doDelete = async (): Promise<void> => {
    ...
    try {
      ...
      setDeletingId(null);
      toast('记录已删除', 'success');
      // refetch both space detail (updates photo grid + session count) and history
      loadData();
    } catch (err) {
      setDeletingId(null);
    ...
    <div className="w-full max-w-4xl">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setConfirmDelete(null)}>
          <div className="mx-4 w-full max-w-sm rounded-card bg-white p-6 shadow-float" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-[16px] font-semibold text-warm">确认删除</h3>
            <p className="mt-2 text-[14px] text-warm-secondary leading-relaxed">
              确定要删除这条整理记录吗？关联的照片和方案也会一并删除，不可恢复。
            </p>
            <div className="mt-5 flex justify-end gap-3">
              <button
                type="button"
                className="rounded-btn border border-border-subtle px-4 py-2 text-[14px] text-warm-secondary hover:bg-cream transition-colors"
              ...
              <button
                type="button"
                className="rounded-btn bg-danger px-4 py-2 text-[14px] font-medium text-white hover:bg-danger/90 transition-colors"
